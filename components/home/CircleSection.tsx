...
import { Stethoscope, ArrowsClockwise, ShieldCheck } from '@/components/ui/Icons';

const FEATS = [
  { icon: <Stethoscope size={18} weight="light" className="text-white/80" />, title: 'Verified Medical Experts',       desc: 'Dermatologists, sports medicine doctors, and Ayurvedic practitioners — answering personally.' },
  { icon: <ArrowsClockwise size={18} weight="light" className="text-white/80" />, title: 'Rotating Specialists Every Week', desc: 'New experts cycle in regularly — so the guidance never stops.' },
  { icon: <ShieldCheck size={18} weight="light" className="text-white/80" />, title: 'Zero Noise. Zero Sponsorship.',   desc: 'A trusted space free from the content that clutters every other wellness community.' },
];
...
export function CircleSection() {
  return (
    <section className="grain py-16 md:py-24 px-4 md:px-10 lg:px-20 bg-[#2C1810]">
            ...
            <div className="flex flex-col gap-3">
              {EXPERTS.map(e => (
                <div key={e.name} className="bg-white/10 rounded-xl px-4 py-3.5 flex items-center gap-3.5 border border-white/10 hover:bg-white/18 transition-all">
                  <div className="w-[42px] h-[42px] rounded-full bg-white/20 flex items-center justify-center flex-shrink-0"><Stethoscope size={20} weight="light" className="text-white/70" /></div>
                  <div>
